...
DeepSeek Harness treats wording and context as first-class engineering concerns, yet the stock Web UI shows no context occupancy and no usage history. `dsh-context-lens` fills both gaps the harness-native way: reading the projections the platform already computes, styling with the platform's design tokens (`--dsw-alias-*`), and mounting through the platform's slot registry — light/dark themes both correct with zero media queries.
...
# then remove the "# dsh-context-lens" block from ~/.dsh/profiles/web/cordis.patch.yml
rm -f ~/.dsh/storages/context-lens-cache.json   # optional: drop the fold cache
```
...
- `sidebar.footer.action` renders the heatmap trigger (label when the sidebar is wide, icon on the rail). The popover fetches `GET /api/context-lens/days` same-origin.

**Host half** (`lib/index.js`): one read-only, loopback-fenced endpoint (`/api/context-lens/days`). Aggregation is incremental: per-session fold state is cached in memory and persisted to `~/.dsh/storages/context-lens-cache.json`; each request folds only events added since the last fold. Live sessions fold their in-memory tail; persisted sessions use the storage backend's opaque revision and `readFrom(id, fromSeq)`, with contiguity checks and a full refold on log rewrites. Steady-state cost stays O(new events) no matter how large the logs grow.
